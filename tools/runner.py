...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            loss_project, sparse_loss, dense_loss = base_model.module.get_loss(ret, gt)
         
            _loss = loss_project * 1e-5 + sparse_loss * 0.1 + dense_loss 
            ...
            if args.distributed:
                ...
                losses.update([sparse_loss.item() , dense_loss.item() * 1000])
            else:
                losses.update([sparse_loss.item() , dense_loss.item() * 1000])
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            if dataset_name == 'PCN':
                ...
                target_path = os.path.join('/data/feiben2/SPO/PCN', 'intermediate0_vis_result')
                ...
                target_path = os.path.join('/data/feiben2/SPO/MVP', 'vis_result')
                if not os.path.exists(target_path):
                    os.mkdir(target_path)
                misc.visualize_KITTI(
                    os.path.join(target_path, f'{model_id}_{idx:03d}'),
                ...
                for item in choice:           
                    ...
                    # # test_metrics.update(_metrics)
                    target_path = os.path.join('/data/feiben2/SPO/ShapeNet55/easy', 'vis_result', taxonomy_id)
                    if not os.path.exists(target_path):
                        os.mkdir(target_path)
                    misc.visualize_KITTI(
                        os.path.join(target_path, f'{model_id}_{idx:03d}'),
                ...
                ret = base_model(partial)
                dense_points = ret[1]
                target_path = os.path.join('/data/feiben2/SPO/KITTI', 'vis_result')
                if not os.path.exists(target_path):
                    os.mkdir(target_path)
                misc.visualize_KITTI(
                    os.path.join(target_path, f'{model_id}_{idx:03d}'),
