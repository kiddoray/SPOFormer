...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            # stat(alexnet, partial)
            
            sparse_loss, dense_loss, imid_loss = base_model.module.get_loss(ret, gt)
         
            _loss = sparse_loss + dense_loss + 0.1 * imid_loss
            _loss.backward()
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            if dataset_name == 'PCN':
                ...
                coarse_points = ret[0]
                dense_points = ret[1]
                target_path = os.path.join('/cpfs2/shared/public/feiben/VQ-DcTr-output/PCN-experiments', 'PCN_vis_result')
                ...
                for item in choice:           
                    ...
                    coarse_points = ret[0]
                    dense_points = ret[1]
                    target_path = os.path.join('/cpfs2/shared/public/feiben/VQ-DcTr-output/ShapeNetUnseen21-experiments', 'PCN-new-vis_result')
                    if not os.path.exists(target_path):
                        os.mkdir(target_path)
                    misc.visualize_KITTI(
                        os.path.join(target_path, f'{model_id}_{idx:03d}'),
