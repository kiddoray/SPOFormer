...
from .dgcnn_group import DGCNN_Grouper, DGCNN_Grouper_up, DGCNN_Grouper_down
...
class PCTransformer(nn.Module):
    ...
    def __init__(self, in_chans=3, embed_dim=768, depth=[6, 6], num_heads=6, mlp_ratio=2., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
        ...
        self.layer3 = nn.Linear(1152, 384)
        self.fusionAttention = fusionAttention(embed_dim, embed_dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop_rate, proj_drop=drop_rate)
        ...
        self.grouper_up = DGCNN_Grouper_up()
        self.grouper_down = DGCNN_Grouper_down()
        ...
        self.pos_embed_down = nn.Sequential(
            nn.Conv1d(in_chans, 16, 1),
            nn.BatchNorm1d(16),
            nn.LeakyReLU(negative_slope=0.2),
            nn.Conv1d(16, embed_dim, 1)
        )
        self.pos_embed_up = nn.Sequential(
            nn.Conv1d(in_chans, 512, 1),
            nn.BatchNorm1d(512),
            nn.LeakyReLU(negative_slope=0.2),
        ...
        self.input_proj_down = nn.Sequential(
            nn.Conv1d(256, embed_dim, 1),
            nn.BatchNorm1d(embed_dim),
            nn.LeakyReLU(negative_slope=0.2),
            nn.Conv1d(embed_dim, embed_dim, 1)
        )
        self.input_proj_up = nn.Sequential(
    ...
    def forward(self, inpc):
        ...
        x = self.input_proj(f).transpose(1,2) #x.shape=[1, 128, 384]
        ...
        for i, blk in enumerate(self.encoder):
            if i < self.knn_layer:
                ...
                encoder_feature.append(x.reshape([bs,-1]))
            else:
                x = blk(x + pos)
                encoder_feature.append(x.reshape([bs,-1]))
        ...
        #
        #print(x.shape) [1, 128, 384]
        #print('x.shape',x.shape)


        #addition
        # coor_up, f_up = self.grouper_up(inpc.transpose(1, 2).contiguous())
        # knn_index_up = get_knn_index(coor_up)
        # pos_up = self.pos_embed_up(coor_up).transpose(1, 2)
        # x_up = self.input_proj_up(f_up).transpose(1, 2)
        # # encoder
        # for i, blk in enumerate(self.encoder):
        #     if i < self.knn_layer:
        #         x_up = blk(x_up + pos_up, knn_index_up)  # B N C
        #     else:
        #         x_up = blk(x_up + pos_up)
        # #x_up = self.layer1(x_up)
        # #print('x_up.shape',x_up.shape)

        # # # addition
        # coor_down, f_down = self.grouper_down(inpc.transpose(1, 2).contiguous())
        # knn_index_down = get_knn_index(coor_down)
        # pos_down = self.pos_embed_down(coor_down).transpose(1, 2)
        # x_down = self.input_proj_down(f_down).transpose(1, 2)
        # # encoder
        # for i, blk in enumerate(self.encoder):
        #     if i < self.knn_layer:
        #         x_down = blk(x_down + pos_down, knn_index_down)  # B N C
        #     else:
        #         x_down = blk(x_down + pos_down)
        # #x_down = self.layer2(x_down)
        # #print('x_down.shape',x_down.shape)

        # channel1 = x
        # att1 = self.fusionAttention(x, x_up, x_up)
        # channel2 = x * att1
        # channel3 = x * self.fusionAttention(x, x_up, x_down)

        # channel_cancat1 = torch.cat((channel1, channel2),2)
        # channel_cancat = torch.cat((channel_cancat1, channel3), 2)
        # #print('channel_cancat', channel_cancat)
        # x = self.layer3(channel_cancat)
        ...
        for i, blk in enumerate(self.decoder):
            if i < self.knn_layer:
                ...
                decoder_feature.append(x.reshape([bs,-1]))
            else:
                q = blk(q, x)
                decoder_feature.append(x.reshape([bs,-1]))
