...
class SkipTransformer(nn.Module):
    def __init__(self, i,  in_channel, dim=256, n_knn=16, pos_hidden_dim=64, attn_hidden_multiplier=4):
        ...
        self.conv_end_1 = nn.Conv1d(dim, in_channel*8, 1)
    ...
    def forward(self, pos, key, query, include_self=True):
        ...
        y_N = y_N.reshape(b, -1, n, 16)
        #print('qk_rel:',qk_rel.shape,'y_N:',y_N.shape)
        add_1 = torch.add(qk_rel,y_N)
        ''''''

        attention = self.attn_mlp(add_1 + pos_embedding)  # b, dim, n, n_knn
        ...
        #print('qk_rel:',qk_rel.shape,'y_N:',y_N.shape,'y:',y.shape)
